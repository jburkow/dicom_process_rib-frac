...
img_folder_path = os.path.join(args.ARGS['8_BIT_FOLDER'],
                               args.ARGS['CROPPED_EQUALIZED_IMAGE_FOLDER'])
imgs = sorted(os.listdir(img_folder_path))

# Load in offset annotation information
anno_df = pd.read_csv(args.ARGS['ANNOTATION_OFFSET_FILENAME'],
                      names=['PatientID', 'x1', 'y1', 'x2', 'y2', 'class'])
...
merged_df.to_csv(args.ARGS['DIMENSION_CHECK_FILENAME'])
